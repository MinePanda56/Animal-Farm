...
#Character Copyright from the book Animal Farm by George Orwell
...
item_pool = [
    ...
    Item("Attack Boosting Amulet", "attack_boost", 5),  # +5 Attack
    Item("Lucky Charm", "crit_rate", 3),  # +3 crit (represents crit rate increase)
    Item("Berserker Stone", "crit_damage", .25),  # +.25 
    ...
    Item("Strength Ring", "attack_boost", 10),  # +10 Attack
    Item("Powerful Elixir", "crit_damage", 0.15),  # +15% Crit Damage
    Item("Shield of Protection", "defense_boost", 10),  # +10 Defense
    Item("Magic Feather", "attack_boost", 8),  # +8 Attack
    Item("Mana Potion", "crit_damage", 0.1),  # +10% Crit Damage
    Item("Phoenix Feather", "heal", 50),  # Heal 50 HP
    Item("Strength Potion", "crit_damage", .2),  # +7 Attack for Crit Attacks
    Item("Combant Manual", "crit_rate", 15),  # +15 crit (represents crit rate increase)
]

# Basic enemies (weaker than named enemies)
basic_enemies = [
    Character("Rabbit", health=70, attack=10, defense=2),
    Character("Goat", health=80  , attack=12, defense=3),
    Character("Chicken", health=45  , attack=8, defense=1),
    Character("Cow", health=100  , attack=7, defense=6),  
    Character("Duck", health=65  , attack=9, defense=1),     
    Character("Turkey", health=70  , attack=14, defense=4),  
]

# Enemy Characters from Animal Farm with special moves
named_enemies = [
    Character("Snowball the Pig", health=200, attack=26, defense=7, special_move="Revolutionary Charge", is_named=True),
    Character("Boxer the Horse", health=200  , attack=25, defense=13, special_move="Mighty Kick", is_named=True),
    Character("Benjamin the Donkey", health=300, attack=14, defense=10, special_move="Cynical Stare", is_named=True),
    Character("Squealer the Pig", health=110  , attack=15, defense=2, special_move="Propaganda Speech", is_named=True),
    Character("The Sheeps", health=130  , attack=12, defense=3, special_move="Chanting Frenzy", is_named=True),
    Character("Old Major's Ghost", health=120  , attack=20, defense=4, special_move="Spirit of Revolution", is_named=True),
    Character("Moses the Raven", health=20 , attack=45, defense=12, special_move="Cloud of Lies", is_named=True),  
    Character("Clover the Horse", health=95 , attack=18, defense=12, special_move="Moral Support", is_named=True),  
    Character("Napoleon's Hound", health=190 , attack=23, defense=8, special_move="Bite", is_named=True),  
    Character("Mr. Jones", health=120 , attack=23, defense=3, special_move="Rile", is_named=True),  
]
Snappy = Character("Snappy the Snapping Turtle", health=999, attack=999, defense=999) 
...
def battle(player, enemy):
    ...
    while player.is_alive() and enemy.is_alive():
        ...
        if enemy.is_alive():
            print(f"\n--- {enemy.name}'s Turn ---")
            enemy_action = random.choice(["attack", "special", "attack"])  # Increased chance for attack
    ...
    if player.is_alive():
        ...
        if enemy.is_named:
            ...
            player.gain_experience(30)  # Basic enemies give less XP
